...
function fixture(): string {
  ...
  cpSync("src/data/profile", join(root, "src/data/profile"), { recursive: true });
  cpSync("docs", join(root, "docs"), { recursive: true });
  return root;
}
function check(root: string): number | null {
  return spawnSync(process.execPath, [resolve("scripts/check-professional-record.mjs")], {
...
describe("professional-record regression gates", () => {
  it("keeps Blossom closed in June 2025 and includes the new credential in both documents", async () => {
    ...
    const blossom = content.experience.find(record => record.id === "blossom-superagent-mentor")!;
    expect(blossom).toMatchObject({dateEnd: "2025-06", current: false});
    expect(formatRecordDates(blossom.dateStart, blossom.dateEnd, blossom.current)).toBe("Nov 2024 to Jun 2025");
    expect(content.credentials.find(record => record.id === "google-project-management")).toMatchObject({issued: "2026-08", verificationCode: "TN8RXZY8M354", status: "completed"});
    for (const document of content.documentManifest) {
      const ids = document.pages.flatMap(page => page.sections.flatMap(section => section.itemIds));
      expect(ids).toEqual(expect.arrayContaining(["google-project-management", "omnimind", "fxpm-validation-system", "ism-2026-validation-gates"]));
    }
  });
  it("preserves verified earlier records without fabricating dates or active status", async () => {
    const content = await loadProfileContent();
    for (const record of [...content.experience, ...content.education].filter(record => record.dateStart === null)) {
      expect(record.current).toBe(false);
      expect(formatRecordDates(record.dateStart, record.dateEnd, record.current, record.dateNote)).not.toContain("Present");
    }
  ...
  it("rejects missing research and skill evidence references", async () => {
    ...
    const result = validateProfileContent(content, { now: new Date("2026-09-08T00:00:00Z") });
    expect(result.errors.filter(error => error.code === "BROKEN_EVIDENCE_REFERENCE")).toHaveLength(2);
  });
  it("rejects semantic data drift and stale review receipts", () => {
    const root = fixture();
    const receiptPath = join(root, "docs/professional-record-review.json");
    const receipt = JSON.parse(readFileSync(receiptPath, "utf8")) as {reviewedOn: string};
    ...
    const records = JSON.parse(readFileSync(path, "utf8")) as {id: string; current: boolean; dateEnd: string | null}[];
    const blossom = records.find(record => record.id === "blossom-superagent-mentor")!;
    blossom.current = true; blossom.dateEnd = null;
